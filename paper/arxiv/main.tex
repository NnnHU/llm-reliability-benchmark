...
\textbf{M1R} single + retry    & Same as M1, but retries on empty/bad output ($\leq$4 attempts)                                 & 1--4 & the contribution of \textbf{retry} alone \\
\textbf{M2} single-model pipeline & extract~$\rightarrow$~analyze~$\rightarrow$~judge, all with one model                       & 3   & the contribution of \textbf{task decomposition} beyond retry \\
\textbf{M3} multi-model Panel+Judge & extract, then 2 models analyze in parallel, then a Judge synthesizes                  & 4   & the contribution of \textbf{multiple models} beyond a single-model pipeline \\
